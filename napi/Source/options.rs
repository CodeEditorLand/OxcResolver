...
pub struct NapiResolveOptions {
	...
	///
	/// For the second value of the tuple, `None -> AliasValue::Ignore`,
	/// Some(String) -> AliasValue::Path(String)`
	...
	/// package. The key order in the exports field is significant. During
	/// condition matching, earlier entries have higher priority and take
	...
	///
	/// Default to `true` when [ResolveOptions::extensions] contains an empty
	/// string. Use `Some(false)` to disable the behavior.
	/// See <https://github.com/webpack/enhanced-resolve/pull/285>
	///
	/// Default None, which is the same as `Some(false)` when the above empty
	/// rule is not applied.
...
impl EnforceExtension {
	pub fn is_auto(&self) -> bool { *self == Self::Auto }
	...
	fn into(self) -> oxc_resolver::TsconfigOptions {
		oxc_resolver::TsconfigOptions {
			...
			references:match self.references {
				...
				Some(Either::B(paths)) => {
					oxc_resolver::TsconfigReferences::Paths(
