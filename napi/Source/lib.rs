...
impl ResolverFactory {
	...
	pub fn new(options: Option<NapiResolveOptions>) -> Self {
		init_tracing();
		let options = options
			.map_or_else(|| ResolveOptions::default(), Self::normalize_options);
	...
	fn normalize_options(op: NapiResolveOptions) -> ResolveOptions {
		...
		ResolveOptions {
			...
			description_files: op
				.description_files
				.unwrap_or(default.description_files),
			...
			extension_alias: op
				.extension_alias
				.map(|extension_alias| {
					extension_alias.into_iter().collect::<Vec<_>>()
				})
			...
			fully_specified: op
				.fully_specified
				.unwrap_or(default.fully_specified),
			main_fields: op
				.main_fields
			...
			resolve_to_context: op
				.resolve_to_context
				.unwrap_or(default.resolve_to_context),
			prefer_relative: op
				.prefer_relative
				.unwrap_or(default.prefer_relative),
			prefer_absolute: op
				.prefer_absolute
				.unwrap_or(default.prefer_absolute),
			restrictions: op
				.restrictions
				.map(|restrictions| {
					restrictions
			...
			builtin_modules: op
				.builtin_modules
				.unwrap_or(default.builtin_modules),
		}
	}
}
