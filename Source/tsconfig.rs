...
impl TsConfig {
	pub fn parse(
		...
		if let Some(base_url) = tsconfig.compiler_options.base_url {
			tsconfig.compiler_options.base_url =
				Some(directory.normalize_with(base_url));
		}
	...
	pub fn extend_tsconfig(&mut self, tsconfig: &Self) {
		...
		if compiler_options.paths.is_none() {
			compiler_options.paths_base =
				compiler_options.base_url.as_ref().map_or_else(
					|| tsconfig.compiler_options.paths_base.clone(),
					Clone::clone,
				);
		...
		if compiler_options.base_url.is_none() {
			compiler_options
				.base_url
				.clone_from(&tsconfig.compiler_options.base_url);
		}
	}
	...
	pub fn resolve_path_alias(&self, specifier: &str) -> Vec<PathBuf> {
		...
		let paths = paths_map.get(specifier).map_or_else(
			|| {
				...
				best_key.and_then(|key| paths_map.get(key)).map_or_else(
					Vec::new,
					|paths| {
						paths
							.iter()
