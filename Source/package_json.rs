...
impl PackageJson {
	...
	/// * query in <https://www.rspack.dev/config/module.html#ruledescriptiondata>
	///   - search on GitHub indicates query on the `type` field.
	///
	/// To reduce overall memory consumption, large fields that useless for
	/// pragmatic use are removed. They are: `description`, `keywords`,
	/// `scripts`, `dependencies` and `devDependencies`, `peerDependencies`,
	/// `optionalDependencies`.
	#[cfg(feature = "package_json_raw_json_api")]
	#[must_use]
	pub const fn raw_json(&self) -> &std::sync::Arc<JSONValue> { &self.raw_json }
	...
	/// The "browser" field is provided by a module author as a hint to
	/// javascript bundlers or component tools when packaging modules for client
	/// side use. Multiple values are configured by
	/// [ResolveOptions::alias_fields].
	///
	...
	/// Resolve the request string for this package.json by looking at the
	/// `browser` field.
	///
	/// # Errors
	///
	/// * Returns [ResolveError::Ignored] for `"path": false` in `browser`
	///   field.
