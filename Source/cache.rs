...
/// An entry in the path cache. Can also be borrowed for lookups without
/// allocations.
enum PathEntry<'a> {
	Owned(CachedPath),
...
impl<Fs:FileSystem> Cache<Fs> {
	...
	pub fn value(&self, path:&Path) -> CachedPath {
		...
		// `path.as_os_str()` hash is not stable because we may joined a path like
		// `foo/bar` and `foo\\bar` on windows.
		let hash = {
		...
		// A DashMap is just an array of RwLock<HashSet>, sharded by hash to reduce lock
		// contention. This uses the low level raw API to avoid cloning the value when
		// using the `entry` method. First, find which shard the value is in, and
		// check to see if we already have a value in the map.
...
impl CachedPath {
	...
	pub fn parent(&self) -> Option<&Self> { self.0.parent.as_ref() }
	...
	fn canocalize_impl<Fs:FileSystem>(&self, cache:&Cache<Fs>) -> Result<Self, ResolveError> {
		// Check if this thread is already canonicalizing. If so, we have found a
		// circular symlink. If a different thread is canonicalizing, OnceLock will
		// queue this thread to wait for the result.
	...
	#[inline]
	#[cfg(not(windows))]
	pub fn normalize_root<Fs:FileSystem>(&self, _cache:&Cache<Fs>) -> Self { self.clone() }
}
