...
impl<Fs> fmt::Debug for ResolverGeneric<Fs> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { self.options.fmt(f) }
}
...
impl<Fs:FileSystem> ResolverGeneric<Fs> {
	...
	/// Returns the options.
	#[must_use]
	pub const fn options(&self) -> &ResolveOptions { &self.options }
	...
	/// * Path to a file without `.json` extension, `.json` will be appended to
	///   filename.
	/// * Path to a directory, where the filename is defaulted to
	///   `tsconfig.json`
	///
	/// # Errors
	///
	...
	fn require_relative(
		...
		debug_assert!(Path::new(specifier).components().next().is_some_and(|c| {
			matches!(c, Component::CurDir | Component::ParentDir | Component::Normal(_))
		}));
	...
	fn load_parse<'s>(
		...
		// There is an edge-case where a request with # can be a path or a fragment ->
		// try both
	...
	fn load_package_imports(
		...
		// 4. let MATCH = PACKAGE_IMPORTS_RESOLVE(X, pathToFileURL(SCOPE), ["node",
	...
	fn load_alias_or_file(&self, cached_path:&CachedPath, ctx:&mut Ctx) -> ResolveResult {
		...
		// Guard this because this is on a hot path, and `.to_string_lossy()` has a
		// cost.
	...
	fn load_node_modules(
		...
		for module_name in &self.options.modules {
			for cached_path in std::iter::successors(Some(cached_path), |p| p.parent()) {
				...
				// 1. Try to interpret X as a combination of NAME and SUBPATH where the name may
				//    have a @scope/ prefix and the subpath begins with a slash (`/`).
				if !package_name.is_empty() {
					...
					if cached_path.is_dir(&self.cache.fs, ctx) {
						...
						// foo/node_modules/package_name is not a directory, so useless to check
						// inside it
						if !subpath.is_empty() {
							continue;
						}
						// Skip if the directory lead to the scope package does not exist
						// i.e. `foo/node_modules/@scope` is not a directory for
						// `foo/node_modules/@scope/package`
	...
	fn load_package_exports(
		...
		//    SUBPATH, `package.json` "exports", ["node", "require"]) defined in the ESM
		//    resolver.
			...
			&& !request.strip_prefix(alias_value).is_some_and(|prefix| prefix.starts_with('/'))
		{
			...
			*should_stop = true;
			...
			return match self.require(cached_path, new_specifier.as_ref(), ctx) {
				Err(ResolveError::NotFound(_) | ResolveError::MatchedAliasNotFound(..)) => Ok(None),
	...
	fn package_exports_resolve(
		...
		if subpath == "." {
			...
			// This is only need when querying the main export, otherwise ctx is passed
			// through.
			...
			let main_export = match exports {
				// 2. If exports is a String or Array, or an Object containing no keys starting with
				//    ".", then
			...
			if let Some(main_export) = main_export {
				// 1. Let resolved be the result of PACKAGE_TARGET_RESOLVE( packageURL,
		...
		if let JSONValue::Object(exports) = exports {
			...
			// 2. Let resolved be the result of PACKAGE_IMPORTS_EXPORTS_RESOLVE( matchKey,
	...
	fn package_imports_resolve(
		...
		// 1. Let resolved be the result of PACKAGE_IMPORTS_EXPORTS_RESOLVE( specifier,
		//    pjson.imports, packageURL, true, conditions).
	...
	fn package_imports_exports_resolve(
		...
		// Error: CachedPath to directories is not possible with the exports field
		// (specifier was ./dist/)
		...
		if !match_key.contains('*') {
			...
			if let Some(target) = match_obj.get(match_key) {
				// 2. Return the result of PACKAGE_TARGET_RESOLVE(packageURL, target, null,
				//    isImports, conditions).
		...
		for (expansion_key, target) in match_obj {
			if expansion_key.starts_with("./") || expansion_key.starts_with('#') {
				// 1. Let patternBase be the substring of expansionKey up to but excluding the
				//    first "*" character.
				if let Some((pattern_base, pattern_trailer)) = expansion_key.split_once('*') {
                        ...
                        && Self::pattern_key_compare(best_key, expansion_key).is_gt()
					{
						...
						// 2. Let patternMatch be the substring of matchKey starting at the index of
						//    the length of patternBase up to the length of matchKey minus the
						//    length of patternTrailer.
						best_match =
					...
					&& Self::pattern_key_compare(best_key, expansion_key).is_gt()
				{
					// TODO: [DEP0148] DeprecationWarning: Use of deprecated folder mapping
					// "./dist/" in the "exports" field module resolution of the package at
					// xxx/package.json.
					best_target = Some(target);
		...
		if let Some(best_target) = best_target {
			// 3. Return the result of PACKAGE_TARGET_RESOLVE(packageURL, target,
		...
		match target {
			...
			JSONValue::String(target) => {
				...
				if !target.starts_with("./") {
					// 1. If isImports is false, or if target starts with "../" or "/", or if target
					//    is a valid URL, then
					...
					//   1. Return PACKAGE_RESOLVE(target with every instance of "*" replaced by
				...
				//    including percent encoded variants, throw an Invalid Package Target error.
			...
			JSONValue::Object(target) => {
				// 1. If exports contains any index property keys, as defined in ECMA-262 6.1.7
				...
				for (key, target_value) in target {
					...
					if key == "default" || conditions.contains(key) {
						...
						// 2. Let resolved be the result of PACKAGE_TARGET_RESOLVE( packageURL,
						//    targetValue, patternMatch, isImports, conditions).
						let resolved = self.package_target_resolve(
							package_url,
							target_key,
							target_value,
			...
			JSONValue::Array(targets) => {
				...
				if targets.is_empty() {
					// Note: return PackagePathNotExported has the same effect as return because
				...
				for (i, target_value) in targets.iter().enumerate() {
					// 1. Let resolved be the result of PACKAGE_TARGET_RESOLVE( packageURL,
					//    targetValue, patternMatch, isImports, conditions), continuing the loop on
					//    any Invalid Package Target error.
					let resolved = self.package_target_resolve(
						package_url,
						target_key,
						target_value,
